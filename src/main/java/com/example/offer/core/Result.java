...
public class Result<T> {
    ...
    public static <D> Result success(BusinessErrorEnum resultCodeEnum){
        log.info("返回码:{} 返回信息:{}", resultCodeEnum.getErrCode(), resultCodeEnum.getErrMsg());
        return new Result<D>().setCode(resultCodeEnum.getErrCode()).setMessage(resultCodeEnum.getErrMsg()).setData(null);
    }

    public static <D> Result success(ResultCodeEnum resultCodeEnum){
        log.info("返回码:{} 返回信息:{}", resultCodeEnum.getCode(), resultCodeEnum.getMessage());
        return new Result<D>().setCode(resultCodeEnum.getCode()).setMessage(resultCodeEnum.getMessage()).setData(null);
    }
